...
program process_mixing_state_new
  ...
  real(kind=dp) :: d_alpha_hyg, d_gamma_hyg, chi_hyg, &
                   ...
                   d_alpha_opt3, d_gamma_opt3, chi_opt3, &
  ...
  !!!!!!!!!!!!!!!!The aerosol species !!!!!!!!!!!!!!!
  type(stats_1d_t) :: stats_chi_hyg, stats_chi_opt1, stats_chi_opt2, stats_chi_opt3, &
                      stats_chi_abd 
  ...
  do i_index = 1,n_index
      ...
      !> Read in mixing state of optical properties based on BC
      call aero_state_mixing_state_metrics(aero_state, aero_data, &
           d_alpha_opt3, d_gamma_opt3, chi_opt3, exclude=(/"H2O"/), group=(/"BC ","OC "/))
      call stats_1d_add_entry(stats_chi_opt3, chi_opt3, i_index)
  ...
  call make_filename(out_filename, prefix, "_mixing_state_new.nc")
  write(*,*) "Writing " // trim(out_filename)
  call pmc_nc_open_write(out_filename, ncid)
  call pmc_nc_write_info(ncid, uuid, "mixing_state_new")
  ...
  call stats_1d_output_netcdf(stats_chi_opt2, ncid, 'chi_opt2', &
         dim_name="time", unit="1")
  call stats_1d_output_netcdf(stats_chi_opt3, ncid, 'chi_opt3', &
         dim_name="time", unit="1")
  ...
  call stats_1d_clear(stats_chi_opt3)
  call stats_1d_clear(stats_chi_abd)

  
  call pmc_mpi_finalize()

end program process_mixing_state_new
